...
There were some limitations to the data and research conducted. The survey was based on feelings and thoughts. It is hard to judge an neighborhoods of homophobic actions if there is no proof of it. Also, a family member being supportive is hard to really see because humans have become good at hiding their true emotions or intentions. You can not base merely off of feelings.

# References

```{r textreg, results="asis", echo=FALSE}
model1 <- lm(happiness~fam_support, data=sjsd)
model2 <- lm(happiness~fam_support+out_family+out_friends+out_neighbors+lgbt_bond+neigh_phobia+event, data=sjsd)
model3 <- lm(happiness~fam_support*out_family+out_friends+out_neighbors+lgbt_bond+neigh_phobia+event, data=sjsd)
```
